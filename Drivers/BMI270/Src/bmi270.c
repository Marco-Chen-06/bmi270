...
//	bmi270_write_byte(hi2c, BMI270_I2C_DEFAULT_ADDR, BMI270_REG_PWR_CONF);
//
//
...
//	bmi270_write(hi2c, BMI270_REG_INIT_DATA, bmi270_config_file, ARRAY_SIZE(bmi270_config_file));
//
//	// complete config load INIT_CTRL = 0x01
//	bmi270_write_byte(hi2c, BMI270_REG_INIT_CTRL, 0x01);
//
//	uint8_t init_status = 0;
//	bmi270_write_byte(hi2c, BMI270_REG_INTERNAL_STATUS, 0x00);
...
int bmi270_write(I2C_HandleTypeDef *hi2c, uint16_t memAddr, const uint8_t *pData, uint16_t size) {
	i2c_err = 0;
	i2c_done = 0;
	HAL_I2C_Mem_Write_IT(hi2c, BMI270_I2C_DEFAULT_ADDR << 1, memAddr, I2C_MEMADD_SIZE_8BIT, (uint8_t*)pData, size);
	if (i2c_wait(hi2c) == -1) {
...
int bmi270_read(I2C_HandleTypeDef *hi2c, uint16_t memAddr, uint8_t *pData, uint16_t size) {
	i2c_err = 0;
	i2c_done = 0;
	HAL_I2C_Mem_Read_IT(hi2c, BMI270_I2C_DEFAULT_ADDR << 1, memAddr, I2C_MEMADD_SIZE_8BIT, pData, size);
	if (i2c_wait(hi2c) == -1) {
		printf("I2C aborted during bmi270_read(). MemAddr: %x. Errcode: %ld \r\n", memAddr, i2c_err);
		return -1;
	}
	return 0;
}
